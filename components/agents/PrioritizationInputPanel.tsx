...
type Props = {
  ...
  onUseSynthesisOpportunitiesChange: (v: boolean) => void;
  onAddOwnOpportunitiesChange: (v: boolean) => void;
  onRun: () => void;
};
...
function runHint(
  ...
  if (!useSynthesis && !addOwn) return "Select at least one input source";
  if (useSynthesis && !synthesisAvailable) return "Run Synthesis on the dashboard first";
  if (addOwn && !rawInput.trim()) {
    return "Add product inputs under your own paste, or enable Synthesis";
  }
  return "Complete the selected input sources";
}

export function PrioritizationInputPanel({
  rawInput,
  productName,
  targetUser,
  ...
  onProductNameChange,
  onTargetUserChange,
  onUseSynthesisOpportunitiesChange,
  onAddOwnOpportunitiesChange,
  onRun,
}: Props) {
  const hint = runHint(
    useSynthesisOpportunities,
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="space-y-2">
        <SourceOption
          title="Use insights from the Synthesis Agent"
          description={
            synthesisAvailable
              ? "Pulls themes, pain points, opportunity insights, and evidence from your latest Synthesis run."
              : "Run the Synthesis Agent first — its output will be available when ready."
          }
          checked={useSynthesisOpportunities}
          disabled={!synthesisAvailable}
          onChange={onUseSynthesisOpportunitiesChange}
        />
        <SourceOption
          title="Add your own product inputs"
          description="Opportunities, pain points, feature ideas, solution concepts, roadmap items, stakeholder requests, or brainstorm notes."
          checked={addOwnOpportunities}
          onChange={onAddOwnOpportunitiesChange}
        />
      </div>

      {addOwnOpportunities && (
        <div className="space-y-4 rounded-[13px] border border-pm-border bg-pm-panel/40 px-4 py-4">
          <SectionLabel>Your product inputs</SectionLabel>
          ...
          <label className="space-y-2 text-sm text-pm-subtle">
            <span className="font-medium text-pm-text">
              Problems, opportunities, features, solutions, or notes
            </span>
            <textarea
              rows={8}
              value={rawInput}
              onChange={(e) => onRawInputChange(e.target.value)}
              placeholder={`One item per line — mix types freely, e.g.\n• Users abandon onboarding at step 3\n• Add CSV export for billing admins\n• Ship mobile push alerts Q3\n• Stakeholder: enterprise SSO by EOY`}
              spellCheck={false}
      ...
      {useSynthesisOpportunities && synthesisAvailable && (
        <p className="rounded-[10px] border border-pm-success/25 bg-pm-success/8 px-3 py-2 text-[12px] text-pm-success">
          Synthesis output from this session will be included to ground scores in evidence.
        </p>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          {loading ? "Running…" : "Run Prioritization Agent"}
        </button>
        {hint && (
